...
char * gettime();
char curtime[SIZE_OF_TIMEBUF];


// 客户端在连接的用户的ip存于此变量中,获取ic的代码在server文件中
char * cli_ip;
#define MAX_FDS 1024
char cli_ips[MAX_FDS][32];
int fd_forLOg;
...
#define Acceess_Log(a,c,l)\
	do{\
		...
		fprintf(log,"%s - %d  [%s] \"%s %s %s\" %d %d\n",cli_ip,fd_forLOg,gettime(),a->http_method,a->http_uri,a->http_version,c,l);\
		fclose(log);\
	}while(0)
...
#define Acceess_Log_Null(c,l)\
	do{\
		log=fopen(filename,"a+");\
		fprintf(log,"%s - %d [%s] \"BAD REQUEST\" %d %d\n",cli_ip,fd_forLOg,gettime(),c,l);\
		fclose(log);\
	}while(0)
...
#define Connect_Log(format,...)\
	do{\
		log=fopen(filename,"a+");\
		fprintf(log,"[ %s - %d ] [%s] " format "\n" \
				,cli_ip,fd_forLOg,gettime(),## __VA_ARGS__);\
		fclose(log);\	
	}while(0)
#else
